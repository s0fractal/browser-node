...
class ConsciousnessDB {
  ...
  // Generic get/set methods for ConsciousnessAPI compatibility
  async get(path) {
    if (!this.db) {
      console.warn('ConsciousnessDB not initialized, returning empty object');
      return {};
    }
    
    // Parse path like "agents/claude/stats" into store and key
    const parts = path.split('/');
    const storeName = parts[0] || 'memory';
  ...
  async set(path, value) {
    if (!this.db) {
      console.warn('ConsciousnessDB not initialized, cannot set value');
      return false;
    }
    ...
    const parts = path.split('/');
    const storeName = parts[0] || 'memory';
    const key = parts.slice(1).join('/') || path;
    
    // Ensure we have a valid store
    const validStores = ['memory', 'intents', 'access', 'mindmap', 'mutations', 'waves', 'agents', 'resonance'];
    const actualStore = validStores.includes(storeName) ? storeName : 'memory';
    
    try {
      // Create store if it doesn't exist (for dynamic stores like 'agents' and 'resonance')
      if (!this.db.objectStoreNames.contains(actualStore)) {
        // For now, use memory store as fallback
      ...
      const tx = this.db.transaction([actualStore], 'readwrite');
